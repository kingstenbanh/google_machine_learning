...
def parse_labels_and_features(dataset):
    ...
    Args:
        dataset: A Pandas `Dataframe`, containing the label on the first column
...
def create_training_input_fn(features, labels, batch_size, num_epochs=None, shuffle=True):
    ...
    Args: 
        ...
        batch_size: Batch size to use during traning
    ...
    def _input_fn(num_epochs=None, shuffle=True):
        ...
        idx = np.randomw.permutation(features.index)
...
def create_predict_input_fn(features, labels, batch_size):
    ...
    """
    def _input_fn():
        raw_features = {"pixels": features.value}
...
def train_linear_classification_model(
    ...
    # Train the model, but do so inside a loop so that we cna periodically assess
    # loss metrics
    ...
    # Remove event files to save disk spaces.
    _ = map(os.remove, glob.glob(os.path.join(classifier.model_dir, 'events.out.tfevents*')))

    # Calculate final predictions (no probabilities, as above).
